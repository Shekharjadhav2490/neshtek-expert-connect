...
public class CustomerRequirementService {
    ...
    public CustomerRequirementService(CustomerRequirementRepository repository,
    ...
    @Transactional
    public Page<CustomerRequirementResponse> list(Pageable pageable) {
        return repository.findAll(pageable).map(this::toResponse);
    }

    @Transactional
    public Page<CustomerRequirementResponse> listByCustomer(Long customerId, Pageable pageable) {
    ...
    public CustomerRequirementResponse update(Long id, CustomerRequirementRequest request) {
        ...
        if (entity.getStatus() != CustomerRequirementStatus.DRAFT
                && entity.getStatus() != CustomerRequirementStatus.SUBMITTED) {
            throw new IllegalStateException("Only DRAFT or SUBMITTED requirements can be edited");
        }
        if (consultationRequestRepository.existsByRequirementId(id)
                || engagementRepository.existsByRequirementId(id)) {
            throw new IllegalStateException("This requirement has already entered the matching or engagement process and can no longer be edited");
        }
        applyRequest(entity, request);
        return toResponse(repository.save(entity));
    }
    ...
    public void delete(Long id) {
        ...
        if (consultationRequestRepository.existsByRequirementId(id)
                || engagementRepository.existsByRequirementId(id)) {
            throw new IllegalStateException("This requirement cannot be deleted because it is already linked to consultation or engagement records");
        }
        repository.delete(entity);
    }

    private CustomerRequirement find(Long id) {
        return repository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Customer requirement not found: " + id));
    }
    ...
    private RequirementPriority parsePriority(String value) {
        ...
        try {
            return RequirementPriority.valueOf(value.trim().toUpperCase(Locale.ROOT));
    ...
    private CustomerRequirementResponse toResponse(CustomerRequirement e) {
        ...
        for (CustomerRequirementSkill s : e.getSkills()) {
            skills.add(new CustomerRequirementResponse.SkillResponse(
                    s.getId(), s.getSkillName(), s.getPriorityOrder(), s.isMandatory()));
        }
        return new CustomerRequirementResponse(
                e.getId(), e.getCustomer() == null ? null : e.getCustomer().getId(),
                e.getCompanyName(), e.getContactName(), e.getEmail(), e.getPhone(),
                e.getCountry(), e.getCity(), e.getTitle(), e.getDescription(), e.getTechnology(),
                e.getRequiredExperienceYears(), e.getEstimatedHours(), e.getPreferredStartDate(),
                e.getPriority().name(), e.getBudget(), e.getCurrencyCode(), e.getStatus().name(),
                e.getCreatedAt(), e.getUpdatedAt(), skills);
    }
}
