              ...
              @for (request of recentConsultations; track request.id) {
                <article class="consultation-card">
                  <div class="consultation-main">
                    ...
                    <h3>{{ request.requirementTitle || ('Requirement #' + request.requirementId) }}</h3>
                    <div class="consultation-meta"><span>Expert: <b>{{ request.expertName || ('Expert #' + request.expertId) }}</b></span><span>Requested: <b>{{ request.createdAt ? (request.createdAt | date:'medium') : '—' }}</b></span></div>
                  </div>
                  <div class="consultation-status-wrap"><span class="status" [class.accepted]="request.status === 'ACCEPTED'" [class.rejected]="request.status === 'REJECTED'" [class.pending]="request.status === 'PENDING'">{{ request.status }}</span>@if (request.respondedAt) { <small>Responded {{ request.respondedAt | date:'mediumDate' }}</small> }</div>
                </article>
              }
            </div>
          }
...
export class CustomerDashboardComponent {
  ...
  loading = true; requirementsLoading = true; consultationsLoading = true; errorMessage = ''; consultationError = ''; successMessage = ''; showCreate = false; saving = false; formError = ''; skillsText = '';
  ...
  constructor() { const user=this.auth.getCurrentUser(); if(!user){this.router.navigateByUrl('/login');return;} this.loadCustomer(user.userId); this.loadConsultations(user.userId); }
  ...
  private loadConsultations(userId:number){this.consultationsLoading=true;this.consultationError='';this.consultationService.listMine(userId).subscribe({next:p=>{this.consultations=p.content??[];this.consultationsLoading=false;},error:e=>{this.consultations=[];this.consultationsLoading=false;this.consultationError=e?.error?.message||e?.error?.error||'Unable to load consultation requests.';}});}
