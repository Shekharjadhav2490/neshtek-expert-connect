            ...
            @for (request of requests; track request.id) {
              <article class="card">
                <div class="head">
                  ...
                  <span class="status" [class.accepted]="request.status === 'ACCEPTED'" [class.rejected]="request.status === 'REJECTED'" [class.pending]="request.status === 'PENDING'">{{ request.status }}</span>
                </div>
                <div class="grid">
                  ...
                  <div><span>Requested</span><b>{{ request.createdAt ? (request.createdAt | date:'medium') : '—' }}</b></div>
...
export class CustomerConsultationsComponent {
  ...
  load(): void {
    ...
    if (!user) { this.loading = false; this.error = 'Your session has expired. Please sign in again.'; return; }
    this.loading = true; this.error = '';
    this.service.listMine(user.userId).subscribe({
      next: page => { this.requests = page.content ?? []; this.loading = false; },
      error: e => { this.loading = false; this.error = e?.error?.message || e?.error?.error || 'Unable to load consultation requests.'; }
    });
  }
}
