...
@Component({
  ...
  template: `
    <main class="page">
      <header class="topbar">
        <div><div class="brand">NeshTek Expert Connect</div><div class="subtitle">Engagement work log</div></div>
        <a class="back" [routerLink]="backPath">Back to dashboard</a>
      </header>

      <section class="hero">
        <div class="eyebrow">Work delivery</div>
        <h1>Work logs</h1>
        <p>Engagement #{{ engagementId }} · Track time delivered against this engagement.</p>
      </section>

      @if (errorMessage) { <section class="state error" role="alert">{{ errorMessage }}</section> }

      <section class="summary">
        <div class="summary-card"><span>Total logged</span><strong>{{ totalHours | number:'1.0-2' }} hrs</strong></div>
        <div class="summary-card"><span>Entries</span><strong>{{ logs.length }}</strong></div>
      </section>
      ...
      @if (isExpert) {
        <section class="panel">
          ...
          <form (ngSubmit)="addLog()">
            <div class="form-grid">
              <label>Work date<input type="date" [(ngModel)]="form.workDate" name="workDate" required></label>
              <label>Hours<input type="number" [(ngModel)]="form.hours" name="hours" min="0.25" max="24" step="0.25" required></label>
            </div>
            <label>Description<textarea [(ngModel)]="form.description" name="description" rows="4" maxlength="2000" placeholder="Describe the work completed..." required></textarea></label>
            <div class="form-actions"><button class="primary" type="submit" [disabled]="saving">{{ saving ? 'Saving…' : 'Add work log' }}</button></div>
          </form>
        </section>
      }

      <section class="panel">
        <div class="section-heading"><div><div class="eyebrow">History</div><h2>Logged work</h2></div><button class="refresh" type="button" (click)="load()" [disabled]="loading">{{ loading ? 'Refreshing…' : 'Refresh' }}</button></div>
        @if (loading) { <div class="state">Loading work logs…</div> }
        @else if (logs.length === 0) { <div class="state">No work has been logged for this engagement yet.</div> }
        @else {
          <div class="logs">
            @for (log of logs; track log.id) {
              <article class="log">
                <div class="date">{{ log.workDate | date:'mediumDate' }}</div>
                <div class="hours">{{ log.hours | number:'1.0-2' }} hrs</div>
                <div class="description">{{ log.description }}</div>
                <div class="created">Added {{ log.createdAt | date:'medium' }}</div>
              </article>
            }
          </div>
        }
...
export class WorkLogsComponent {
  ...
  constructor() {
    if (!this.engagementId) { this.errorMessage = 'Invalid engagement.'; this.loading = false; return; }
    this.load();
  }

  load(): void {
    this.loading = true; this.errorMessage = '';
    const logs$ = this.isExpert ? this.expertService.getWorkLogs(this.engagementId) : this.customerService.getWorkLogs(this.engagementId);
    const total$ = this.isExpert ? this.expertService.getTotalWorkHours(this.engagementId) : this.customerService.getTotalWorkHours(this.engagementId);
    logs$.subscribe({ next: page => { this.logs = page.content ?? []; this.loading = false; }, error: error => { this.loading = false; this.errorMessage = error?.error?.error || 'Unable to load work logs.'; } });
    total$.subscribe({ next: total => this.totalHours = Number(total ?? 0), error: () => this.totalHours = 0 });
  }

  addLog(): void {
    if (!this.form.workDate || !this.form.hours || !this.form.description.trim()) return;
    this.saving = true; this.errorMessage = '';
    this.expertService.addWorkLog(this.engagementId, { ...this.form, description: this.form.description.trim() }).subscribe({
      next: () => { this.saving = false; this.form = { workDate: new Date().toISOString().slice(0, 10), hours: 1, description: '' }; this.load(); },
      error: error => { this.saving = false; this.errorMessage = error?.error?.error || 'Unable to save the work log.'; }
    });
  }
}
