  ...
  template: `
    <section class="replacement-panel">
      <div class="panel-head">
        <div><div class="eyebrow">Replacement tracking</div><h3>Expert replacement</h3><div class="summary">{{ items.length ? items.length + ' replacement request' + (items.length === 1 ? '' : 's') + ' recorded' : 'No replacement activity loaded' }}</div></div>
...
export class ReplacementHistoryPanelComponent implements OnChanges {
  ...
  get canRequest(): boolean { if (!this.allowRequest || !['READY','ACTIVE','PAUSED'].includes(this.engagementStatus)) return false; return !this.items.some(item => item.status === 'REQUESTED' || item.status === 'APPROVED'); }
