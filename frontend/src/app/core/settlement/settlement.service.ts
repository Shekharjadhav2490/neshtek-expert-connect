...
export interface Settlement {
  ...
  status:'REQUESTED'|'CUSTOMER_APPROVED'|'APPROVED_FOR_PAYOUT'|'PAID'|'REJECTED'|string;
  requestedAt:string; customerApprovedAt:string|null; customerRejectedAt:string|null; customerComment:string|null;
  approvedAt:string|null; paidAt:string|null; rejectedAt:string|null; adminComment:string|null; paymentReference:string|null;
}
...
export class SettlementService {
  ...
  byCustomer(customerId:number):Observable<SettlementPage>{return this.http.get<SettlementPage>(`${this.url}/customers/${customerId}?page=0&size=50`);}
  byEngagement(engagementId:number):Observable<Settlement>{return this.http.get<Settlement>(`${this.url}/engagements/${engagementId}`);}
  ...
  customerApprove(id:number,comment=''):Observable<Settlement>{return this.http.post<Settlement>(`${this.url}/${id}/customer-approve?comment=${encodeURIComponent(comment)}`,{});}
  customerReject(id:number,reason:string):Observable<Settlement>{return this.http.post<Settlement>(`${this.url}/${id}/customer-reject?reason=${encodeURIComponent(reason)}`,{});}
