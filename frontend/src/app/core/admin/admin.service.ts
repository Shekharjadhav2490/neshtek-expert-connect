...
export interface AdminCustomer { customerId: number; companyName: string; contactName: string; email: string; status: string; }
...
export class AdminService {
  ...
  getCustomers(page = 0, size = 20): Observable<AdminPage<AdminCustomer>> { return this.http.get<AdminPage<AdminCustomer>>(`${this.baseUrl}/customers?page=${page}&size=${size}`); }
  getExperts(page = 0, size = 20, status?: string): Observable<AdminPage<AdminExpert>> { const params = new URLSearchParams({page:String(page),size:String(size)}); if(status) params.set('status',status); return this.http.get<AdminPage<AdminExpert>>(`${this.baseUrl}/experts?${params.toString()}`); }
